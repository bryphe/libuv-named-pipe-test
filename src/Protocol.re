...
~namedPipe: string,
~initData: InitData.t,
~dispatch: Message.t => unit,
) => {

	let transport = ref(None);

	let send = (packet) => switch(transport^) {
	...
	let transportHandler = msg  => switch(msg) {
	...
	let resTransport = Transport.start(
	~namedPipe,
	~dispatch=transportHandler,
	);

	resTransport
	|> Result.iter(t => transport := Some(t));

	resTransport
	|> Result.map(_ => { transport: transport });
}
